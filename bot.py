...
def m_turi(update: Update, context: CallbackContext):
    ...
    btns = [
        ["Термо наклейкалар","бирка етикиткалар"],
        ["Дукон аксесуарлари","Пресс дазмол"],
        ["Лазер","Вишивка"],
        ["Безак тош","Принтер"],
        ["Печат (шелкография)"]
    ]
...
def saving(update: Update, context: CallbackContext):
    ...
    update.message.reply_text("Yana malumotlarni to'ldirish uchun /adding komandasini bosing bosing.")
...
def cancel(update: Update, context: CallbackContext):
    user_data = context.user_data
    user_data.clear()
    update.message.reply_text("Ma'lumot to'ldirish bekor qilindi.Qayta boshlash uchun /adding buyrug'ini bosing.")
    return ConversationHandler.END



updater=Updater('6557277705:AAHWeNRzvuaicVr0Kumwa0CM8wQ4Q2-woCU')


dp = updater.dispatcher
dp.add_handler(CommandHandler('start',start))
dp.add_handler(MessageHandler(Filters.text('Statistikam🗒'),my_statistik))

    # ConversationHandlerni qo'shish
conv_handler = ConversationHandler(
    entry_points=[CommandHandler('adding',first)],
    states={
